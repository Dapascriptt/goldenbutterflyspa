...
@section('content')
    <div class="rounded-2xl bg-white border border-[#eadfce] p-6">
        <h2 class="text-lg font-semibold text-[#4b2f1a]">Stock Out: {{ $item->name }}</h2>
        <p class="mt-1 text-sm text-slate-500">Periode: {{ $period['label'] }}</p>

        <form method="POST" action="{{ route('manage.inventory.stock-out', $item->id) }}" class="mt-6 space-y-4">
            @csrf
